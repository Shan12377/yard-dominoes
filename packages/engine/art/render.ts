...
function tileSvg(tile: DiagramTile): string {
  ...
  return `<g transform="translate(${tile.x} ${tile.y}) rotate(${tile.rotate ?? 0} 28 56)" opacity="${opacity}">
...
${(spec.badges ?? []).map((badge) => `<g transform="translate(${badge.x} ${badge.y})"><rect x="-55" y="-18" width="110" height="36" rx="18" fill="${tone(badge.tone)}"/><text text-anchor="middle" y="6" fill="${COLOR.pip}" font-family="system-ui,sans-serif" font-size="15" font-weight="800">${esc(badge.text)}</text></g>`).join('')}
