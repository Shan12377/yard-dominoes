...
2026-09-15 17:05 (v155) — Owner feedback batch ("I need perfection"). Shipped:
- Practice pass the pose: in partnered games the winning side's poser gets "Keep it / Pass to partner" before the first bone, never on hand one of a set or when the double-six is forced (engine `passPoseToPartner`). Verified in a 6-hand desktop run.
- Top face-down hand sits inside the table again (hang-off reverted); racks and my hand keep a fixed modest bone on desktop.
- Result heading no longer under the table (room height follows the table).
- Desktop double-six boards (cut throat, partner, open hand, Across): Practice seat list moved under the table (its width is wood now); the board takes the band between the top rack and my hand, full width between the side players; every hand turns its centre row after 4 bones; bone sized with the real 2-domino climb; a long hand re-lays a size smaller down to 11px, never clipped (Across used to stop at 22px and hide bones). 1440x900 Practice: 88px bones (was 76), 1920: 116-128px. 3-6 hand runs at 1280/1366/1440/1920: 0 clipped, 0 covered, 0 moves during play. Across hand panels compacted.
- Desktop French: chucha centred between the top rack and my hand, legs left/right 4, up/down 3 (turn early only when the height runs out), bones bigger but capped by viewport height so up/down keep two bones and a double: 19px unit at 1440x900 (was 18), 25px at 1920x1080 (was 24).
Known limit told to the owner: on a laptop-height screen, 25-30% bigger French bones and 3 straight up/down bones cannot both fit between the top rack and the hand.
Resume point: owner's look at v155 on desktop; Lounge desktop keeps its chat rail, so its bones are smaller (68px at 1440); Supabase dashboard steps for Google/email still with the owner.

2026-09-15 21:05 (v161 pending) — Owner batch: flicker, French bones, Lounge colour, Across climb, side info.
- Flicker (v160, live): portraits, played bones, my own bones, the sticker/quick-chat bars and the You panel are kept across rebuilds. Measured at a live Lounge table: created <img> elements fell from ~800 per 25s to 16; new bone elements to the bones actually played. Profile drafts (name/location/origin/gender) survive a rebuild, which is why a typed name used to revert.
- French bones now use the same design as every other game: the French-only phone override (own border, padding, pip size, pip colour, divider) is gone and French joins `.line.phone-route .tile`. French DIRECTION of play is untouched — it is deliberately different (owner, 2026-09-15).
- The table colour picker is now at the Lounge table too, next to Sound (one saved setting app-wide). The header row wraps so Leave table cannot be pushed off a phone screen.
- Across climbs three dominoes between rows (ACROSS_CLIMB), so its turns sit clear of the middle and the board reads fuller: desktop 1440 route 20:57x16:3 then 21:54x19:3, nothing clipped or moved. Measured over the 400 sizing hands, the third bone costs nothing at these sizes.
- The turn line and Pass moved out of the middle into a narrow strip down my own side of the table (`hand-side-info`, placed by measurement into the tallest free gap in that column, so it never lands on a rack or identity card). The pace chooser stays with the bones. Desktop cut throat keeps its 23px bone and gains rows (22 -> 23/24); Across gained a size (20 -> 21).
Still open: French centre row shows 3-4 across in the Lounge rather than 7 (arms turn early); the owner's "one domino up then across" change for the up arm; one double still shifts a few pixels late in a Lounge French hand.
