...
- Academy visual teaching is complete for the current curriculum. All 33
  lessons have deterministic SVG table diagrams generated by
  `scripts/gen-diagrams.ts`; lessons open one at a time using See it / Why it
  matters / Take it to the table / Try it. All 11 declared drills now present
  an answerable position, one unambiguous choice, immediate explanation and a
...
- Duppies use a stable one-based seat label wherever a player follows the
  ...
  solver's proof. The `review-hand` service rebuilds legacy cached reviews
  once, without charging the player, so this teaching evidence reaches hands
  already reviewed before the Coach update. French and Across now have visual
...
- A seated player's one controlled hand belongs in the protected lower rail
  inside the felt, with the board renderer fitting only the stage above it.
  This makes the whole table readable as one object without allowing a played
  bone to overlap an interactive tile. Its temporary decision controls (end
  choice, pass, reshuffle) dock directly below the felt so they never shrink
  the board; only the board stage may scroll on an extreme device. Spectators
  have no local rail; Across retains both controlled hands below the felt
  because compressing two hands into one rail would harm the board-first
  layout.
