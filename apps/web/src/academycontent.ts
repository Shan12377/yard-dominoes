...
export const ACADEMY_VISUALS: Record<string, AcademyVisual> = {
  ...
  B2L2: { alt: 'The double-six holder opens a set and lays the six itself; every other opening tile is refused.', notice: 'A set always starts on the six — the holder of it opens, and that is the only legal first move.', takeaway: 'Hold the six at the start of a set and you open with it. After that hand, the winner poses what he likes.', tryIt: 'You hold 6-6 on the first hand of a set. Can you open 2-5 instead?', answer: 'No. The six opens a set, so 6-6 is your only legal move.' },
  ...
  B5L6: { alt: 'A tournament table with four real players seated, the six already led to open the round.', notice: 'A tournament is an event you enter, not a different way to play — the rules are the rules you already know.', takeaway: 'Match play asks for etiquette, not new rules: one speed, bones down only when you lay them, disputes to the deal check.', tryIt: 'A tournament round is drawn but one seat is empty. Does a computer player fill it?', answer: 'No. A tournament is played by real people — the seat waits for a substitute.' },
};
...
const SCENARIOS: Record<string, DrillScenario> = {
  ...
  B3D1: { setup: 'You won the last hand, so the pose is yours and you may lead anything. Your hand includes 5-5, 4-5, 2-5, 1-5, 3-6, 2-4, 0-1.', visual: {
